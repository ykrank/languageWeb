...
(function()
    local function init()
        ...
        不入栈
        @登录之后让登录页面不入栈,公共短信不入栈
        ]]
        function Stack:push(value)
            if (value=="LOGIN001" and tiaozhuan=="0") then
--if (value=="LOGIN001" and tiaozhuan=="0") or value=="ebank_public" then
                return self:size()
          -- elseif (value=="ebank_public") then
        ...
        function Query:show(modal_flag)
            ...
            if type(self._ctrls[1]) == "userdata" then
              ...
              if type(self._selector) == "string" then
                ...
                self._next_tag = self._next_tag + 2;
              end
              ert:debug_ert("page tag: ".. tag);
...
(function()
    local function init()
        ...
        function Page:replace(transiton_type,nopush)
            ...
            if nopush then
                location:replace(self._content, nil, self.replace_callback_nopush,self);
            else
                location:replace(self._content, nil, self.replace_callback,self);
            end
        end
...
(function()
    local function init()
        ...
        function Channel:insert_request(request)
        ...
        function channel_factory.success(response)
          ...
          local resbody = response["responseBody"];
          if string.find(resbody,"^%s*%{") ~= nil then
            local table=json:objectFromJSON(resbody) or {};
            table["header"]=table["header"] or {};
            local tran_success = table["header"]["tran_success"];
            local error_code = table["header"]["error_code"];
            if tran_success == "1" then
              channel_factory:hide_loading();
              if error_code == "1" then
                window:alert(table["header"]["error_msg"]);
                return;
              elseif error_code == "2" then
                window:alert("该功能需要绑卡后才能继续交易，是否立即去绑卡？","是","否",
                  function(index)
                    if index == 1 then
                      --否
                      return;
                      ...
                      --是
                      channel_factory:first_page("ebank_tied_card","TCM00001",{id="ebank_tied_card",tranCode="TCM00001"},{});
                      return;
                    end
                  end);
                return;
              elseif error_code == "3" then
                  --自定义全局变量--
                  global_hx_rate = "";--华兴宝七日收益率
                  global_zn_rate = "";--智能存款年化收益率
                  is_login = nil;--判断是否登录
                  obligate = nil;--预留信息
        ...
        function channel_factory:first_page(id, trancode, post_body, options)
          if is_login~="1" and (trancode=="MANU0001" or trancode=="MSG00001" or trancode=="TCM00001" or trancode=="EMAS0001" or trancode=="PD0002" or trancode=="PX0002") then
              local context = {id=id,trancode=trancode,post_body=post_body};
              window:alert("登录之后才能进行下一步操作，您是否要登录", "确定", "取消",function(index)
                      if index == 0 then
                          tiaozhuan="0";
                          ert.channel:first_page("ebank_login_register","LOGIN001",{id="ebank_login_register",tranCode="LOGIN001"},{just_page=true,context=context});
                      end
                  end);
              return;
          else
        ...
        function channel_factory:next_page(id, trancode, post_body, options)
            ...
            if is_login~="1" and (trancode=="MANU0001" or trancode=="MSG00001" or trancode=="TCM00001" or trancode=="EMAS0001" or trancode=="PD0002" or trancode=="PX0002") then
                ...
                self.current_id=id;
                channel:insert_request(post_body);
        ...
        -- @param trancoe 生成page对象的id
        -- @parma page_content 页面加载需要的报文

        function channel_factory:replace(trancode,page_content)
            local channel = self:get_channel();
        ...
        function channel_factory.do_back(channel,page)
            page:replace(channel.options.back_trasiton_type,true);
        end;
        -----------------------------------
        -- 返回channel的上一个界面，如果以是当前channel的第一页，则返回到进入channel的界面
        -- page_stack维持当前页面在栈顶
        function channel_factory:back()
            ert:debug_ert(self.channel_stack);
            local channel = self:get_channel();
            if channel~= nil then
                ert:debug_ert(channel.page_stack:size());
                local pop_page = channel:pop_page();
                if pop_page ~= nil then
                  local page = channel:get_page();
                  if page ~= nil then
                      channel_factory.do_back(channel,page);
                  else
        ...
        function channel_factory:finish()
