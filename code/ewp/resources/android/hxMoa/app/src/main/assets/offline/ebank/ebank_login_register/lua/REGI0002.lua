...
--获取上个页面输入的数据
     local last_page_example = ert.channel:get_page("ebank_login_register","REGI0002");
     ...
     local checkType = this.context["checkType"];
     local eAccountNo = this.context["eAccountNo"];
...
--注册信息填写下一步
      function this.next_page()
          if checkType =="6" then
              local loginPwd = ert("#lg_pwd"):val();
              if loginPwd == "" then
                 window:alert("请设置登录密码");
                 return;
              end
              ...
              local loginPwd2 = ert("#lg_pwd2"):val();
              if loginPwd2 == "" then
                 window:alert("请确认登录密码");
                 return;
              end
              if loginPwd ~= loginPwd2 then
                 window:alert("登录密码与确认登录密码必须保持一致");
                 return;
              end
              ...
              if mobile ~= phone then
                  window:alert("您输入的手机号与获取短信的手机号不一致");
                  return;
              end
              local post_body={id='ebank_login_register',tranCode='REGI0003',loginPwd=loginPwd,loginPwd2=loginPwd2,mobile=mobile,securityCode=securityCode,tradePwd=tradePwd,tradePwd2=tradePwd2,recommend=recommend,username=username,cardId=cardId,custno=custno,eAccountNo=eAccountNo,checkType=checkType}
          
              ert.channel:next_page("ebank_login_register","REGI0003",post_body,{});
          else
              local loginPwd;
              local loginPwd2;
              if checkType ~="2" then
                  loginPwd = ert("#lg_pwd"):val();
                  if loginPwd == "" then
                     window:alert("请设置登录密码");
                     return;
                  end
...
end)(ert.channel:get_page("ebank_login_register","REGI0002"));
